...
_jump:
	...
	mov bp,sp
	mov bx,[bp+4]
	; get the segment to jump to into bx
	push ds ; save ds as it will be modified below
	push si
	mov ax,cs
	; modify the jmp below to jump to the specified segment
	mov ds,ax
	; this is self-modifying code
	mov si,#jump
	mov [si+3],bx
	; change the first 0000 to the segment
	pop si
	pop ax ; save segment of the caller in ax (= ds which was pushed
	above)
	mov cx,sp ; save stack pointer
	mov dx,bp ; save base pointer
	mov ds,bx
	; set up the segment registers to the specified segment
	mov ss,bx
	mov es,bx
...
jump:
call #0x0000:0x0000 ; start running (the first 0000 is changed above)
...
_exit:
	; pop what is pushed by the compiler at the beginning of the program's
main() function
	; required because when exit() is called the code never reaches the end of
the main() function
